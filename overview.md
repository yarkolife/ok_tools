...
- **Interactive admin notification center**: The `notifications` app now provides a responsive React work surface matching the rental process UI. Rich event rows expose the translated event explanation and useful payload details, while a bottom drawer keeps the subject, state, timing, description and object-specific action together. Five overview cards filter the table to today's expectations, new events, open actions, postponed items or problems; the expectations panel remains prominent alongside the table. Facts are stored as `NotificationEvent` rows with a deduplication key, while expectations ("equipment due back today") are derived from current data on every render and shown as read-only rows, so a moved date leaves no stale event. Links open the affected object or the exact rental, exchange or planning working context. Delivery is still page rendering only: no emails, polling or websockets. Three gate levels stay separate — module flag, per-channel switch and personal subscription — and visibility mirrors `ModelAdmin.has_view_permission`. The JSX bundle is part of the production `build:js` pipeline and is collected with the other static assets during deployment.
