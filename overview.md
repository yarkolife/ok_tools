...
- **Admin notifications — daily summary and event feed (stage 1)**: A new `notifications` app puts one place in the admin where staff see what happened and what is expected today. Delivery is the rendering of a page: no emails, no polling, no websockets, so it cannot spam. Facts are stored as `NotificationEvent` rows with a deduplication key; expectations ("equipment due back today") are derived from current data on every render and never stored, so a moved date leaves no stale row. Event types are declared in code and mirrored into a configuration table after `migrate`, which means a new type — including its numeric parameters — needs no migration. Three gate levels stay separate: the module flag in settings, a per-channel switch (a disabled check is not executed at all), and the personal subscription, which can never re-enable what the channel switched off. Visibility mirrors `ModelAdmin.has_view_permission` rather than `user.has_perm`, so a subscription cannot widen access. The daily task at `NotificationConfig.digest_time` marks the start of the day and produces the events no signal can produce; its schedule is synchronised into django-celery-beat from the admin.
...
- **Key Features**:
  - Summary block on the admin start page and a personal feed page
