...
# Checking and fixing problematic migrations
print_info "Checking migrations..."
if [ -f "$INSTALL_DIR/media_files/migrations/0004_add_unc_path_to_storage.py" ]; then
    print_warning "Found problematic migration 0004_add_unc_path_to_storage.py"
    print_info "Removing problematic migration..."
    rm -f "$INSTALL_DIR/media_files/migrations/0004_add_unc_path_to_storage.py"
    print_success "Problematic migration removed"
fi
