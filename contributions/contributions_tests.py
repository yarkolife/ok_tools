...
def test__contributions__view__ListContributionsView__2(browser):
    ...
    assert 'Test Title' not in browser.contents
...
def test__contributions__admin__2(browser, disaimport):
    """DISA import state can be marked without creating contributions."""
    browser.login_admin()
    browser.open(disa_url_change(disaimport.id))

    assert 'Imported' not in browser.contents
    disaimport.imported = True
    disaimport.save(update_fields=['imported'])
...
def test__contributions__admin__PrimaryFilter__1(
    ...
    filter = PrimaryFilter({}, {}, Contribution, ContributionAdmin)

    with patch.object(filter, 'value', return_value='y'):
        result = filter.queryset(None, queryset)
    assert primary in result
    assert repetition1 not in result
    assert repetition2 not in result

    with patch.object(filter, 'value', return_value='n'):
        result = filter.queryset(None, queryset)
    assert primary not in result
    assert repetition1 in result
    assert repetition2 in result
...
def test__contributions__admin__ContributionResource__1(
    ...
    export = ContributionResource().export(Contribution.objects.all()).csv

    assert str(contr1.broadcast_date.date()) in export
    assert str(contr2.broadcast_date.date()) not in export
...
def test__contributions__admin__ContributionResource__3(
    ...
    assert str(contr1.broadcast_date.date()) in export
    assert str(contr1.broadcast_date.time()) in export
...
def test__contributions__admin__ProgramResource__1(
    ...
    export = ProgramResource().export(Contribution.objects.all()).csv

    assert str(contr1.broadcast_date.time()) in export
    assert (str((contr1.broadcast_date + license1.duration).time())
            in export)
    assert license1.title in export

    assert str(contr2.broadcast_date.time()) in export
    assert (str((contr2.broadcast_date + license2.duration).time())
            in export)
    assert license2.title in export

    assert str(contr3.broadcast_date.time()) in export
    assert (str((contr3.broadcast_date + license2.duration).time())
            in export)

    assert str(time(hour=0, minute=0)) in export
    assert 'Info block' in export
...
def test__contributions__disa_import__disa_import_partial_success(mocked_request, license):
    ...
    valid_row = [MagicMock(value="") for _ in range(12)]
    valid_row[1].value = "08.09.2022 09:30:00"
    valid_row[2].value = "08.09.2022 10:00:00"
    valid_row[3].value = "00:30:00"
    valid_row[4].value = f"{license.number}_Valid Title"
    valid_row[11].value = "Live-Quelle"
    invalid_row = [MagicMock(value="") for _ in range(12)]
    invalid_row[1].value = "invalid date"
    invalid_row[2].value = "08.09.2022 11:00:00"
    invalid_row[3].value = "00:30:00"
    invalid_row[4].value = "456_Invalid Title"
    invalid_row[11].value = "Live-Quelle"
    mock_rows = [valid_row, invalid_row]
    ...
    with patch('contributions.disa_import.load_workbook') as mock_load_workbook:
        ...
        with patch('contributions.disa_import._extract_license_number', side_effect=lambda x: int(x.split('_')[0]) if x.startswith((f'{license.number}_', '456_')) else None):
